...
def stop():
    stop_url = url + "resourceid/%s/sid/%s/mode/mix/stop" % (
        request.json["resourceId"],
        request.json["sid"],
    )
    stop_body = {"uid": "99",
                 "cname": request.json["cName"], "clientRequest": {}}
...
def query():
    query_url = url + "resourceid/%s/sid/%s/mode/mix/query" % (
        request.json["resourceId"],
        request.json["sid"],
    )
